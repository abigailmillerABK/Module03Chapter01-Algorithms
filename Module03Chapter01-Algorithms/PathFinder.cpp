...
void PathFinder::addPath(char start, char end, int cost) {
...
Node* PathFinder::GetNodeByChar(char nodeChar) {
	std::list<Node*>::iterator charPtr;
...
int PathFinder::findPath(char start, char end) {
	clearData();
	//set start dist = to 0
...
void PathFinder::MakeKnown(Node* thisNode) {
	std::list<Node*>::iterator charPtr;
